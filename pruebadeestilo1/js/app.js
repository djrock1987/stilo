...
(function ($) {
	...
	OB = {
		...
		currentGuide: "circular",
		imageObject: new Image(),
		guidesObject: new Image(),
		...
		glassesInstanceScaled: false,
		glassesInstanceRotated: false,
		glassesInstanceMoved: false,
		...
		fbProfilePictures: [],
		errorByStep: {
			'1-0': 'Por favor seleccion tu estilo',
			'2-0': 'Por favor selecciona un metodo para cargar tu foto',
			'2-1': 'Por favor carga una imagen',
			'2-2': 'Por favor selecciona una foto de tu albúm',
			'2-3': 'Por favor tomate una foto',
		...
		construct: function () {
			...
			OB.guidesObject.crossOrigin = "anonymous";
			OB.guidesObject.onload = function () {
				OB.faceCanvas.renderAll();
			}
			OB.guidesObject.src = jsObject.baseUrl + 'guias/' + OB.currentGuide + '.png';
		},
		...
		canvasObjectRotated: function (event) {
			...
			switch (obj.id) {
				case 'faceInstance':
					...
					if (angle < 180) {
						OB.slider['rotate'].slider("value", angle + 180);
					} else {
						OB.slider['rotate'].slider("value", angle - 180);
					}
					OB.faceInstanciated = true;
				...
				case 'glassesInstance':
					...
					OB.glassesInstanceRotated = true;
					OB.glassesInstanciated = true;
					break;
			}
		},
		checkFace: function () {
			...
			if (objects.length > 0) {
				...
				$.each(objects, function (key, object) {
					if (object.id == 'faceInstance') {
						OB.checkFacePosition(object);
					}
				})
			}
		},
		checkFacePosition: function (obj) {
			...
			var maxPosTop = size.height / 2;
			var minPosTop = ((2 * OB.canvasHeight) - size.height) / 2;

			var maxPosLeft = size.width / 2;
			var minPosLeft = ((2 * OB.canvasWidth) - size.width) / 2;

			if (OB.faceInstancePosition.top > maxPosTop) {
				OB.faceInstancePosition.top = maxPosTop;
				obj.setTop(OB.faceInstancePosition.top);
			}
			if (OB.faceInstancePosition.top < minPosTop) {
				OB.faceInstancePosition.top = minPosTop;
				obj.setTop(OB.faceInstancePosition.top);
			}

			if (OB.faceInstancePosition.left > maxPosLeft) {
				OB.faceInstancePosition.left = maxPosLeft;
				obj.setLeft(OB.faceInstancePosition.left);
			}
			if (OB.faceInstancePosition.left < minPosLeft) {
				OB.faceInstancePosition.left = minPosLeft;
				obj.setLeft(OB.faceInstancePosition.left);
			}
		},
		...
		goToStep: function (step, sub_step, has_substep, is_final_step) {
			if (!has_substep) {
				if (!is_final_step) {
					...
					//Procesar paso final
					document.location = jsObject.baseUrl;
				}
			} else {
				//Procesar sub step
				if (sub_step === 0) {
		...
		setGradosActivacion: function () {
			...
			var setGradoActivacion = function (key) {
				...
				OB.filterInstance.setOpacity(grados[key].opacidad/100);
...
//				var srcImage = jsObject.baseUrl + '/monturas/' + OB.currentStyle + '/' + (parseInt(OB.currentGlassesKey) + 1) + '/' + OB.currentColor + '.png';
//
//				if (srcImage !== '') {
//					var image = new Image();
...
//						OB.setGlassesFilter(grados[key].opacidad, function () {
//
//
//						});
//					};
...
//				}
			...
			var temp = 0;
			$.each(OB.availableStyles[OB.currentStyle].filters, function (key, val) {

				grados[temp] = {
					grado: key,
					opacidad: val.opacidad
				};
				temp++;
			});
			totalFilters = grados.length;
		...
		getMonturasByStyle: function (style) {
			...
			if (style !== '') {
				for (i = 0; i < OB.availableStyles[style].total; i++) {
					images[i] = {
						picture: jsObject.baseUrl + 'monturas/' + style + '/originales/' + (i + 1) + '.png',
						style: style,
						key: i
					};
				}
				...
				$.each(OB.availableStyles, function (keyStyle, style) {
					...
					for (i = 0; i < style.total; i++) {
						images[keyImg] = {
							picture: jsObject.baseUrl + 'monturas/' + keyStyle + '/originales/' + (i + 1) + '.png',
							style: keyStyle,
							key: i
						};
						keyImg++;
		...
		setGlassesFilter: function (opacidad, callback) {
			...
			if ($('#step-6-0 .image-selected img').length === 0) {
				...
				OB.filterInstance = new fabric.Image(OB.filterObject, {
					top: (maxHeight / 2) - 30,
					left: maxWidth / 2,
				...
				if (OB.glassesInstanceScaled) {
					OB.filterInstance.scale(OB.glassesInstanceAttributes.scale);
				}

				if (OB.glassesInstanceRotated) {
					OB.filterInstance.setAngle(OB.glassesInstanceAttributes.angle);
				}

				if (OB.glassesInstanceMoved) {
					OB.filterInstance.setLeft(OB.glassesInstanceAttributes.left);
					OB.filterInstance.setTop(OB.glassesInstanceAttributes.top);
				}

				OB.faceCanvas.add(OB.filterInstance);

				if (typeof callback !== 'undefined') {
					callback(OB.filterInstance);
				}

			}
		},
		removeGlasses: function () {
			var objects = OB.faceCanvas.getObjects();
			if (objects.length > 0) {
		...
		setImage: function () {
			...
			if ($('#step-3-0 .image-selected img').length === 0) {
				...
				OB.faceInstance.scale((OB.slider['zoom'].slider("option", "value") + 100) / 100);
		...
		addGuides: function () {
			OB.guiasInstance = new fabric.Image(OB.guidesObject, {
				selectable: false,
				perPixelTargetFind: true,
				id: 'guiasInstance'
			});
			OB.faceCanvas.add(OB.guiasInstance);
		},
					...
					if (objects.length > 0) {
						...
						$.each(objects, function (key, object) {
							if (object.id != 'guiasInstance') {
								...
								object.hasControls = object.hasBorders = true;
							}
						});
					}
					OB.faceCanvas.renderAll();
		...
		stepOneActions: function () {
			$('#step-1-0 .style-selection').on('click', function (event) {
				...
				styleImage.src = jsObject.baseUrl + 'images/g-' + style + '.jpg';
		...
		stepThreeActions: function () {
			$('.guideSelection').click(function (event) {
				event.preventDefault();
				var guide = $(this).data('guide');
				if (guide !== OB.currentGuide) {
					OB.currentGuide = guide;
					OB.guidesObject.src = jsObject.baseUrl + 'guias/' + OB.currentGuide + '.png';
				}
		...
		stepFourActions: function () {
			...
			$('.facebook-pic').on('click', '#monturas-photos .photos-grid li a', function (event) {
				...
				var srcImage = jsObject.baseUrl + '/monturas/' + OB.currentStyle + '/' + (parseInt(OB.currentGlassesKey) + 1) + '/montura.png';
		...
		resetForm: function (actionVars) {
			$('#' + actionVars.object_id).trigger('reset');
			$('#' + actionVars.object_id).find('select').val('')
			$('#' + actionVars.object_id).find('select').trigger('change');
			$('#' + actionVars.object_id).find('.options label').removeClass('active')
		},
		...
		startGetUserMedia: function () {
			...
			navigator.getUserMedia({
				...
				var objStri = OB.currentStep + '-' + OB.currentSubStep;
				OB.showError($('#step-' + objStri), error.message);
			});
		...
		slidersObjects: function () {
			...
			$.each($('.bar-element'), function (key, val) {
				...
				OB.slider[type].slider({
					range: "min",
					value: (type === 'zoom') ? 0 : 180,
					min: 0,
					max: (type === 'zoom') ? 100 : 360,
					step: 5,
					slide: function (event, ui) {
						if (type === 'rotate') {
							...
							OB.faceInstance.scale((ui.value + 100) / 100);
							OB.faceCanvas.renderAll();
							OB.checkFace();
						}
					},
